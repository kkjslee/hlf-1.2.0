...
CORE_PEER_ADDRESS=peer0.org1.example.com:7051 peer chaincode install -n marbles -v 0.1 -p $CHAINCODE_DIR/marbles

CHAINCODE_DIR=github.com/hyperledger/fabric/examples/chaincode/go/src
CORE_PEER_ADDRESS=peer1.org1.example.com:7051 peer chaincode install -n marbles -v 0.1 -p $CHAINCODE_DIR/marbles
...
-C mychannel -n marbles -v 0.1 -c '{"Args":["init"]}' -P "OR   ('Org1MSP.peer','Org2MSP.peer')"
